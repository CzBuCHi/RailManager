...
// run `dotnet stryker` only on changed projects, then merge report into single json and html
...
void Execute(string[] args) {
    ...
    var solution = strykerConfig.RootElement.GetProperty("stryker-config").GetProperty("solution").GetString()
                   ?? throw new("Unable to resolve solution from stryker-config.json");
...
static string Save(JsonDocument report) {
...
static JsonDocument MergeReports(string solutionFullPath, JsonDocument?[] reports) {
    if (reports.Length == 0) {
...
static IEnumerable<TestTestedPairs> EnumerateTestTestedPairs(ProjectInfo[] projects) {
    ...
    foreach (var project in dict.Values.Where(o => o.Name.EndsWith(".Tests"))) {
        if (dict.TryGetValue(project.Name[..^6], out var testedProject)) {
            yield return new(testedProject.Name, testedProject.Path, project.Path);
        }
    }
}

static JsonDocument? GetTestReport(string solutionFullPath, TestTestedPairs pair, string[] args) {
...
static string GetReportPath(string identifier) =>
    Path.Combine(Directory.GetCurrentDirectory(), "StrykerOutput", identifier, "reports", "mutation-report.json");

static bool RunStryker(string solutionFullPath, TestTestedPairs pair, string[] args) {
...
static bool IsReportOutdated(TestTestedPairs pair) {
    var reportDate = GetReportDate(pair.Identifier);
    string[] excludes = [
        Path.DirectorySeparatorChar + "bin" + Path.DirectorySeparatorChar,
        Path.DirectorySeparatorChar + "obj" + Path.DirectorySeparatorChar
    ];
    var newestProjectFile = GetNewestFileInDirectory(new(Path.Combine(Directory.GetCurrentDirectory(), Path.GetDirectoryName(pair.Project)!)), excludes);
    var newestTestProjectFile =
        GetNewestFileInDirectory(new(Path.Combine(Directory.GetCurrentDirectory(), Path.GetDirectoryName(pair.TestProject)!)), excludes);
...
static DateTime? GetReportDate(string identifier) {
    var file = new FileInfo(GetReportPath(identifier));
    return file.Exists ? file.LastWriteTime : null;
}

static FileInfo? GetNewestFileInDirectory(DirectoryInfo directory, string[] excludes) =>
    directory.Exists
...
internal record ProjectInfo(string Name, string Path);

internal record TestTestedPairs(string Identifier, string Project, string TestProject);
