...
namespace RailManager.Services;
...
public sealed class MemoryLogger : IMemoryLogger
{
    ...
    public void Flush(ILogger logger) {
        ...
        logger.Information(
            "All log messages before this one were logged before the logger was configured. Their timestamps may be invalid.");
    }

    private static void FlushMessage(object message, ILogger logger) {
        switch (message) {
            case LogEvent logEvent: logger.Write(logEvent); break;
            case LogMessage simple:
                logger.Write(simple.Level, simple.Exception, simple.MessageTemplate, simple.PropertyValues); break;
        }
    }
    ...
    public void Write<T>(LogEventLevel level, string messageTemplate, T propertyValue) =>
        Write(level, null!, messageTemplate, [propertyValue]);

    public void Write<T0, T1>(LogEventLevel level, string messageTemplate, T0 propertyValue0, T1 propertyValue1) =>
        Write(level, null!, messageTemplate, [propertyValue0, propertyValue1]);
    ...
    public void Write<T0, T1, T2>(
        ...
        T1 propertyValue1,
        T2 propertyValue2
    ) => Write(level, null!, messageTemplate, [propertyValue0, propertyValue1, propertyValue2]);

    public void Write(LogEventLevel level, string messageTemplate, params object?[] propertyValues) =>
        Write(level, (Exception)null!, messageTemplate, propertyValues);

    public void Write(LogEventLevel level, Exception exception, string messageTemplate) =>
        Write(level, exception, messageTemplate, []);
    ...
    public void Write(
        ...
        string messageTemplate,
        params object?[] propertyValues
    ) => _LogMessages.Add(new LogMessage(level, exception, messageTemplate, propertyValues));
